...
public class HomeActivity extends AppCompatActivity implements DroneController.DroneStateListener {
  ...
  private void testDroneConnection() {
    if (droneController != null) {
      ...
      } else if (droneController.isSimulationMode()) {
        Toast.makeText(this, "시뮬레이션 모드로 실행 중입니다.", Toast.LENGTH_SHORT).show();
      } else {
    ...
    runOnUiThread(() -> {
      ...
      switch (state) {
        ...
        case CONNECTING:
          ...
          break;
        case CONNECTED:
          statusText += droneController.isSimulationMode() ? "연결됨 (시뮬레이션)" : "연결됨 (실제 드론)";
          break;
        case DISCONNECTED:
          statusText += "연결 해제됨";
          break;
